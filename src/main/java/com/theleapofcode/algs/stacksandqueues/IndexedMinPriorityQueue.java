...
/**
 * The {@code IndexedMinPriorityQueue} class represents an indexed priority
 ...
 * <em>change-the-key</em> methods. In order to let the client refer to keys on
 * the priority queue, an integer between {@code 0} and {@code maxN - 1} is
 * associated with each key—the client uses this integer to specify which key to
 * delete or change. It also supports methods for peeking at the minimum key,
 ...
 */
public class IndexedMinPriorityQueue<K extends Comparable<K>> implements Iterable<Integer> {
	...
	private class HeapIterator implements Iterator<Integer> {
		// create a new pq
		private IndexedMinPriorityQueue<K> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copy = new IndexedMinPriorityQueue<>(pq.length - 1);
			for (int i = 1; i <= n; i++)
