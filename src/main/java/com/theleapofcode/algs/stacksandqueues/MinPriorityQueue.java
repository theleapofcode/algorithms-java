...
public class MinPriorityQueue<T extends Comparable<T>> implements Queue<T> {
	...
	private void sink(int k) {
		while (2 * k < size) {
			...
			if (j < size - 1 && great(j, j + 1)) // j holds greater of children
				j++;
			if (!great(k, j)) // Compare parent and greater of children
				break;
			exch(k, j);
			k = j;
		}
	...
	private class HeapIterator implements Iterator<T> {
		...
		private MinPriorityQueue<T> copyQueue;

		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copyQueue = new MinPriorityQueue<>(size);
			for (int i = 0; i < size; i++)
