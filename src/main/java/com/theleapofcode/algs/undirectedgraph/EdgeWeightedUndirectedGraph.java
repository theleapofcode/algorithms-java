...
/**
 * The {@code EdgeWeightedUndirectedGraph} class represents an edge-weighted
 * graph of vertices named 0 through <em>V</em> – 1, where each undirected edge
 * is of type {@link Edge} and has a real-valued weight. It supports the
 ...
 * number of vertices <em>V</em> and the number of edges <em>E</em>. Parallel
 * edges and self-loops are permitted. By convention, a self-loop
 * <em>v</em>-<em>v</em> appears in the adjacency list of <em>v</em> twice and
 * contributes two to the degree of <em>v</em>.
 * <p>
...
public class EdgeWeightedUndirectedGraph {
	...
	public EdgeWeightedUndirectedGraph(int V) {
		...
		this.V = V;
		this.E = 0;
		adj = (BagLinkedListImpl<Edge>[]) new BagLinkedListImpl[V];
		for (int v = 0; v < V; v++) {
	...
	public EdgeWeightedUndirectedGraph(EdgeWeightedUndirectedGraph G) {
		...
		for (int v = 0; v < G.V(); v++) {
			...
			Stack<Edge> reverse = new StackLinkedListImpl<>();
			for (Edge e : G.adj[v]) {
				reverse.push(e);
			}
			for (Edge e : reverse) {
	...
	public Iterable<Edge> edges() {
		Bag<Edge> list = new BagLinkedListImpl<>();
		for (int v = 0; v < V; v++) {
			int selfLoops = 0;
			for (Edge e : adj(v)) {
				if (e.other(v) > v) {
